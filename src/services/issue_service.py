...
class IssueService:
    ...
    def __init__(self, project_name: Optional[str] = None, project_key: Optional[str] = None):
        ...
        if not project_name and not project_key:
            ...
            logger.warning("No project specified, using fallback default: %s", self.project_name)
        else:
            logger.info("Initializing IssueService with project_name=%s, project_key=%s",
    ...
    async def create_issue(
        ...
        """
        logger.info("Creating issue: title=%s, priority=%s, assignee=%s",
    ...
    async def get_issue(self, issue_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to get issue details: id=%d, error=%s", issue_id, e, exc_info=True)
            raise
