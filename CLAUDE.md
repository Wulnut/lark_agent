...
本项目配置了以下按需加载的 Skills（位于 `.claude/skills/`）：

| Skill | 描述 | 调用方式 |
|-------|------|---------|
| `python-dev` | Python 编码规则和最佳实践 | `/python-dev` |
| `typescript-dev` | TypeScript 编码规则和最佳实践 | `/typescript-dev` |
| `cpp-dev` | 现代 C++ (C++17/20) 编码规则 | `/cpp-dev` |
| `git-commit` | Git 提交规范和分支管理 | `/git-commit` |
| `gitflow` | Gitflow 工作流规则 | `/gitflow` |
| `document` | Markdown 文档编写规范 | `/document` |
| `riper-5` | RIPER-5 严格模式协议 | `/riper-5` |
| `async-python-patterns` | Python asyncio 异步编程模式 | `/async-python-patterns` |
| `code-reviewer` | 综合代码审查工具 | `/code-reviewer` |
| `skill-creator` | 创建自定义 Skills 指南 | `/skill-creator` |
